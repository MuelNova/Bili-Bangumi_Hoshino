...
async def sub(bot,ctx):
    ...
    if re.match('查[看询]*番剧订阅',msg):
        o = SQL('Subscription')
        bgms = [k for k in o.iterkeys() if ctx['group_id'] in o.get(k)]
        o = SQL('All_Sub')
        await bot.send(
            ctx,
            '群{}番剧订阅列表:\n🎀🎀全部推送:{}\n🎀'.format(
                str(ctx['group_id']), ctx['group_id'] in o.get('data')
            )
            + '\n🎀'.join(bgms),
        )
        ...
        o = SQL('All_Sub')
        gid = ctx['group_id']
        grps = o.get('data') or set()
        grps.add(gid)
        o['data'] = grps
        await bot.send(ctx, f'已经为群{str(gid)}添加全部订阅')
    elif msg == '番剧订阅 !all':
        o = SQL('All_Sub')
        gid = ctx['group_id']
        grps = o.get('data') or set()
        grps.discard(gid)
        o['data'] = grps
        await bot.send(ctx, f'已经为群{str(gid)}取消全部订阅')
        ...
        await bot.send(
            ctx,
            '番剧列表:\n🎀'
            + '\n🎀'.join(f'{i + 1}.{st[i]}' for i in range(len(st))),
        )
        ...
        if msg.isdigit() and int(msg) - 1 <= len(st) and int(msg) >= 1:
            o = SQL('Subscription')
            bgm = st[int(msg)-1]
            grp = o.get(bgm) or set()
...
async def update_report():
    ...
    if not cache or (len(cache) != len(new_data)):
        ...
        for i in range(len(new_data)):
            if new_data[i].get('is_published') and not cache[i].get(
                'is_published'
            ):
                cache = new_data
                k = new_data[i]
                sv.logger.info(f"检测到番剧更新:{k.get('title')}")
                msg = MessageSegment.image(k.get('cover'))
                msg = msg + '🎀{}\n   更新了 {} \n\n{}'.format(k.get('title'),k.get('pub_index'),k.get('url'))
                o = SQL('Subscription')
                grps = o.get(k.get('title')) or set()
                o = SQL('All_Sub')
                grps2 = o.get('data') or set()
                grps = list(grps | grps2)
                for i in grps:
                    await bot.send_group_msg(group_id=i,message='有番剧更新啦(*•̀ᴗ•́*)و\n\n' + msg)
